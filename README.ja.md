...
- **10 秒ほどデータが来ない** — ダッシュボードがデータ途絶を表示し、30 秒続くと
  ...
  ありますが、正常です。接続後も最初の計測値が 60 秒間来ない場合は接続を作り直します。
- **共有を開始しないまま Start した** — BLE 接続を一度も確立できない状態が連続
  30 分続くと、無期限にスキャンし続けないようブリッジだけを自動停止します。
  アプリとトレイは終了しないため、共有を開始してから再度 Start できます。一度でも
  接続できた実行にはこの30分制限は適用されず、心拍受信中は時間制限なく動作します。
- **ウィンドウの × ボタン** — 既定ではアプリを終了せず、トレイに隠れて動作を続けます。
  設定でこの動作をオフにすると × ボタンで安全な停止処理後に終了します。明示的に終了
  する場合はトレイメニューの「終了」を使用してください。
